...
async def main():
    print(f"[Boost-{PROTO} 50000] Starting ProxRipper {PROTO.upper()} booster SECOND 50k...")
    ...
    print(f"[Boost-{PROTO}] Fetching ProxRipper {PROTO.upper()} SECOND 50k ({SKIP_FIRST}-{SKIP_FIRST+MAX_PROXIES})...")
    text = ""
    try:
    ...
    print(f"[Boost-{PROTO}] Fetched {len(all_proxies)} total unique, using SECOND 50k: {SKIP_FIRST}-{SKIP_FIRST+len(proxies)} ({len(proxies)} proxies)")
